...
# -------------------------- CONFIG --------------------------
csv_file = "falling_sleeping_a.csv"
MIN_SEGMENT_LENGTH = 4          # discard segments shorter than this
...
class_to_label = {'Sleeping': 0, 'Falling': 1}
...
# -------------------------- Stats & Plot --------------------------
Sleeping_segments = segment_df[segment_df['label'] == 0]
Falling_segments  = segment_df[segment_df['label'] == 1]

Sleeping_lengths = Sleeping_segments['segment_length']
...
print(f"  Sleeping : {len(Sleeping_lengths):6,d}  ({len(Sleeping_lengths)/len(segment_df)*100:5.1f}%)")
...
for label, name in [(0, 'Sleeping'), (1, 'Falling')]:
...
plt.hist(Sleeping_lengths, bins=50, alpha=0.6, label='Sleeping(0)', color='dodgerblue', edgecolor='navy')
plt.hist(Falling_lengths,  bins=50, alpha=0.6, label='Falling (1)',  color='salmon', edgecolor='darkred')
